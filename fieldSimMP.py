...
def GetTerms():
    ...
    P = InspectStatesR(p, m.H_sp[signature])

    sig = 1
    found = False

    N_terms = len(m.H_sp[signature])

    while(not(found)):
        signature = (n, p + sig)
        m.H_sp[signature] = []
        P += InspectStatesR(p +sig, m.H_sp[signature])

        if len(m.H_sp[signature]) == 0:
            del m.H_sp[signature]
        else:
            m.tags.append(str(signature))
